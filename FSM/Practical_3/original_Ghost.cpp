#include "Ghost_main.H"


// Global Variables

double x0 = 0, x1 = 1, T = 0.0007, GammaL = 1.4, GammaR = 1.2;
int nxCells = 400;
...
int main()
{
    ...
    for (int i = 0; i != nxCells+2; i++)
    {
        ...
        u0[i][0] = ((x < 0.5) ? 1 : 1);
        u0[i][1] = ((x < 0.5) ? 0 : 0);
        u0[i][2] = ((x < 0.5) ? 1e5: 1e5);
    }
    
    for (int i = 0; i != nxCells+2; i++)
    {
        double x = x0 + (i-0.5)*dx;
        u1[i][0] = ((x < 0.5) ? 0.125 : 0.125);
        u1[i][1] = ((x < 0.5) ? 0 : 0);
        u1[i][2] = ((x < 0.5) ? 1e4 : 1e4);
    }
...
void solver_Original_Ghost(mat &u0, mat &u1, vector<double> &phi0, const double &GammaL, const double &GammaR, string method)
{
    ...
    do{
        computeGhostFluidBoundaries(u0, u1, phi, GammaL, GammaR);
