program quadrature_gather
  ...
  real(kind(1d0)), allocatable :: totals(:)
...
call mpi_gather(total, 1, mpi_double_precision, totals, 1, mpi_double_precision, 0, mpi_comm_world)

if (rank .eq. 0) then
   do i = 0, nproc - 1
      result = result + totals(i+1)
   end do
