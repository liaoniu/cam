...
program mandel
  ...
  integer :: ix,iy,res,i, j, c, k, iters
  integer,  allocatable :: pallette(:,:)
  integer :: colors(3, 9)
  integer, allocatable :: set(:)
  integer mand
  ...
  do i=1,8
     do j = 1 + ((i-1) * iters)/8, (i * iters)/8
        ...
        do c = 1, 3
           pallette(c, j) = colors(c, i) + nint((colors(c, i+1) - colors(c, i)) * var)
        end do
     end do
  end do
  ...
  do iy=0,res-1
     ...
     do ix=0,res-1
        ...
        do k = 0, 2
           set(3 * res * iy + 3 * ix + k + 1)=pallette(k+1,mand(cmplx(x,y,kind(1d0)),iters))
        end do
     enddo
  enddo

  call ppm_write(set,10,"m.ppm", res)
