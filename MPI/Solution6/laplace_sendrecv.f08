program laplace_sendrecv

  use mpi_f08

  real (kind(1d0)), pointer, dimension(:,:) :: g1, g2, p
  integer :: i, j, k, img, ht, n, niter, rank, nproc, source, dest
  ...
  do n = 1, niter
     ...
     call mpi_sendrecv(g1(:, ht - 1), size, mpi_double_precision, dest, &
          1, g1(:, 1), size, mpi_double_precision, source, mpi_any_tag, &
          mpi_comm_world, st)
     ...
     call mpi_sendrecv(g1(:, 2), size, mpi_double_precision, dest, 1, &
          g1(:, ht), size, mpi_double_precision, source, mpi_any_tag, &
          mpi_comm_world, st)
