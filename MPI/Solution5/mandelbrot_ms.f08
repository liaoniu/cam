...
program mandelbrot_ms
  ...
  integer :: ix,iy,res,i, j, k, iters, rank, nproc, counter, r, s, msg
  ...
  if (rank .eq. 0) then
     ...
     do i = 0, res-1
        call mpi_recv(myset, 3 * res, mpi_integer, mpi_any_source, mpi_any_tag, mpi_comm_world, st)

        r = st%mpi_tag
        s = st%mpi_source

        allsets(3 * res * r + 1 : 3 * res * (r+1)) = myset
     ...
     do
        ...
        if (i .eq. -1) then
           print *, 'Rank ', rank, ' has computed ', counter, ' rows. '
           exit
        end if
        ...
        do j=0, res-1
           ...
           do k = 0,2
              myset(3 * j + k + 1) = pallette(k + 1, mand(cmplx(x,y,kind(1d0)),iters))
           end do
        end do

        counter = counter + 1

        call mpi_send(myset, 3 * res, mpi_integer, 0, i, mpi_comm_world)
     end do
  end if
  
  call mpi_finalize()
end program mandelbrot_ms
