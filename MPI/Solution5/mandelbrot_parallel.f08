...
program mandel
  ...
  integer :: ix,iy,res,i, j, k, iters, rank, nproc, istart, iend
  integer,  allocatable :: pallette(:,:), myset(:), allsets(:)
  integer :: colors(3, 9)
  integer, allocatable :: starts(:), ends(:), lengths(:), displs(:)
  integer mand
  ...
  allocate(starts(nproc))
  allocate(ends(nproc))
  allocate(lengths(nproc))
  allocate(displs(nproc))

  if (rank == 0) then
     displs(1) = 0
     do i = 0, nproc-1
        starts(i+1) = (i * res) / nproc
        ends(i+1) = (i + 1) * res / nproc
        lengths(i+1) = 3 * res * (ends(i+1) - starts(i+1))
        if (i .gt. 0) displs(i+1) = displs(i) + lengths(i)
     end do
     allocate(allsets(3 * res * res))
  end if

  call mpi_scatter(starts, 1, mpi_integer, istart, 1, mpi_integer, 0, mpi_comm_world)
  call mpi_scatter(ends, 1, mpi_integer, iend, 1, mpi_integer, 0, mpi_comm_world)

  allocate(myset(3 * (iend-istart) * res))
  ...
  do iy=istart,iend-1
     ...
     do ix=0,res-1
        ...
        do k=0,2
           myset(3 * res * (iy-istart) + 3 * ix + k + 1)=pallette(k+1,mand(cmplx(x,y,kind(1d0)),iters))
        end do
     enddo
  enddo

  call mpi_gatherv(myset, (iend-istart) * 3 * res, mpi_integer, allsets, lengths, displs, mpi_integer, 0, mpi_comm_world)
