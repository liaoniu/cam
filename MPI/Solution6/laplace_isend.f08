program laplace_send
  ...
  do n = 1, niter
     ...
     call mpi_waitall(2, reqs, stats)

     if (rank .ne. nproc - 1) then
        call mpi_isend(g1(:, ht - 1), size, mpi_double_precision, rank + 1, 1, mpi_comm_world, reqs(1))
     end if

     if (rank .ne. 0) then
        call mpi_isend(g1(:, 2), size, mpi_double_precision, rank - 1, 1, mpi_comm_world, reqs(2))
     end if
