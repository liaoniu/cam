program laplace_send
  ...
  allocate (mpi_buff(size*4))
  
  call mpi_buffer_attach(mpi_buff, 4*size*int(sizeof(1d0)))
  ...
  do n = 1, niter
     ...
     if (rank .ne. nproc - 1) then
        call mpi_bsend(g1(:, ht - 1), size, mpi_double_precision, rank + 1, 1, mpi_comm_world)
        call mpi_recv(g1(:, ht), size, mpi_double_precision, rank + 1, mpi_any_tag, mpi_comm_world, st)
     end if

     if (rank .ne. 0) then
        call mpi_bsend(g1(:, 2), size, mpi_double_precision, rank - 1, 1, mpi_comm_world)
     end if
