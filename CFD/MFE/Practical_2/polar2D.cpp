...
array<double, 4> s(array<double, 4> &u, double r, double &gamma);
...
mat operatorSplit(mat &u, double &dt, double &r0, double dr, int &nxCells, int &nyCells, double &gamma, string method);

bool inCircle(double x, double y, double R);
void transmissiveBC(mat& u, int nxCells, int nyCells);
void halfReflectiveBC(mat&u, int nxCells, int nyCells);

bool inCircle(double x, double y, double R){
    return ((x-1)*(x-1) + (y-1)*(y-1) < R*R) ? true : false;
}

bool inSphere(double r, double z, double R){
    return (r*r + z*z < R*R) ? true : false;
}
...
array<double, 4> s(array<double, 4> &u, double r, double &gamma){
    array<double, 4> s_res;
    double rho, vr, vz, p;
    double rhoVr, rhoVz, E;
    tie(rho, rhoVr, rhoVz, E) = tie(u[0], u[1], u[2], u[3]);
    vr = rhoVr/rho;
    vz = rhoVz/rho;
    p = (gamma - 1)*(E - 0.5*(rhoVr*vr + rhoVz*vz));
    s_res[0] = -rhoVr/r;
    s_res[1] = -rhoVr*vr/r;
    s_res[2] = -rhoVr*vz/r;
    s_res[3] = -(E + p)*vr/r;
...
tuple<mat, mat> halfTimeUpdate(mat &uL, mat &uR, double gamma, double dx, double dy, double dt, string direction){
    ...
    int nyCells = uL[0].size()-3;
    if (direction == "x"){
        for (int i = 0; i != nxCells+3; i++){
            for (int j = 0; j != nyCells+3; j++){
...
mat split(mat &u, mat &flux, double dt, double &dx, double &dy, int &nxCells, int &nyCells, double &gamma, string direction, string method){
    ...
    halfReflectiveBC(u, nxCells, nyCells);
    return u;
}

mat operatorSplit(mat &u, double &dt, double &r0, double dr, int &nxCells, int &nyCells, double &gamma, string method="Heun"){
    if (method == "Heun"){
        mat K1 = u, K2 = u;
        for (int i = 0; i != nxCells+3; i++){
            double r = r0 + (i-1.5)*dr;
            for (int j = 0; j != nyCells+2; j++){
                K1[i][j] = s(K1[i][j], r, gamma);
                for (int var = 0; var != 4; var++){
                    K1[i][j][var] *= dt;
                }
            }
        ...
        for (int i = 0; i != nxCells+3; i++){
            for (int j = 0; j != nyCells+2; j++){
                for (int var = 0; var != 4; var++){
                    K2[i][j][var] = K2[i][j][var] + 0.5*K1[i][j][var];
                }
            }
        }
        for (int i = 0; i != nxCells+3; i++){
        ...
        for (int i = 0; i != nxCells+3; i++){
            for (int j = 0; j != nyCells+2; j++){
                for (int var = 0; var != 4; var++){
                    u[i][j][var] = u[i][j][var] + 0.5*(K1[i][j][var] + K2[i][j][var]);
                }
            }
        }
    }
...
void solver(mat &u0, double T, double x0, double x1, double y0, double y1, int nxCells = 100, int nyCells = 100, double gamma = 1.4, string method = "FORCE"){
    ...
    mat flux;
    mat u = u0;
    flux.resize(nxCells+2, vector<array<double, 4>> (nyCells+1));
    ...
    do{
        ...
        //     }
        // }
        operatorSplit(u, dt, x0, dx, nxCells, nyCells, gamma);
    ...
    for (int i = 1; i != nxCells + 2; i++){
        double x = x0 + (i-1.5)*dx;
        output1 << x << " " << u[i][501][0] << endl;
        output2 << x << " " << u[i][501][1] << endl;
        output3 << x << " " << u[i][501][2] << endl;
        output4 << x << " " << u[i][501][3] << endl;
    }
    output1.close();
    output2.close();
    output3.close();
...
int main()
{
    ...
    double r0 = 0, r1 = 1, z0 = -1, z1 = 1, T = 0.25, gamma = 1.4, nrCells = 1000, nzCells = 1000;
    double dr = (r1 - r0)/nrCells;
    double dz = (z1 - z0)/nzCells;
    mat u0;
    u0.resize(nrCells+3, vector<array<double, 4>> (nzCells+2));
    for (int i = 0; i != nrCells+3; i++){
        double r = r0 + (i-1.5)*dr;
        for (int j = 0; j != nzCells+2; j++){
            double z = z0 + (j-0.5)*dz;
            u0[i][j][0] = (inSphere(r, z, 0.4) ? 1 : 0.125);
            u0[i][j][1] = (inSphere(r, z, 0.4) ? 0 : 0);
            u0[i][j][2] = (inSphere(r, z, 0.4) ? 0 : 0);
            u0[i][j][3] = (inSphere(r, z, 0.4) ? 1 : 0.1);
        }
    }
    ...
    solver(u0, T, r0, r1, z0, z1, nrCells, nzCells, gamma, "FORCE");
    return 0;
